...
<div id="widgetmenu" class="widgetmenu <?=$menu_type?>" style="--radius:<?=$radius?>px; --animation-length:<?=$animation_length/1000?>s; <?if($menu_width != 0) echo 'width:'.$menu_width.'px;';?>">
	<ul>
      ...
      if( $row['me_img'] && strpos($row['me_img'], '.') != '' ){
        // 이미지에 값이 존재하고 .이 존재하는 이미지 링크라면
      if($row['me_img2']){
      $img_link="<img class='widgetmenu_img' src=\"{$row['me_img']}\" onmouseenter=\"this.src='{$row['me_img2']}'\" onmouseleave=\"this.src='{$row['me_img']}'\" >";
      }else{
      $img_link='<img class="widgetmenu_img" src="'.$row['me_img'].'">';
      }
      } else if ( $row['me_name'] && strpos($row['me_img'], '.') == '' ) {
        // 이미지에 값이 존재하고 이미지 링크가 아니라면
        $icon_style = explode(' ', $row['me_img']);
        if ($icon_style[0] == 'duotone' || $icon_style[0] == 'light' || $icon_style[0] == 'regular' || $icon_style[0] == 'solid' || $icon_style[0] == 'thin' || $icon_style[0] == 'brands'){
          ...
          $img_link='<i class="material-icons widgetmenu_icon">'.$row['me_img'].'</i>';
        }
      }
      ...
      if($member['mb_level']>=$row['me_level']){?>
          <li class="widgetmenu_list <?=$menuitem_type?>" >
              <div class="widgetmenu_item">
                ...
                <?php if($row['me_img'] || $menu_type == 'iconic_circle'){
      ...
      <?} else if($member['mb_level']<$row['me_level'] && $secret_type == 'false') {?>
        <li class="widgetmenu_list <?=$menuitem_type?>" style="opacity:0.5; pointer-events: none;">
              ...
              <div class="widgetmenu_item">
                ...
                <?php if($row['me_img']) echo $img_link; else echo ''; ?>
                </div>
                <div class="widgetmenu_item_text">
                <?php if($row['me_name']) echo $me_name; else echo ''; ?>
                </div></a>
